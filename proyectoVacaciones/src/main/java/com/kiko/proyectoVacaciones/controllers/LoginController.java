...
public class LoginController {
	...
	@Autowired
	EmployeeService usuarioServicio;
	...
	public String register(@ModelAttribute Employee employee,  @RequestParam("file") MultipartFile file) {		
		if (!file.isEmpty()) {
			String imagen = storageService.store(file);
			employee.setImage(MvcUriComponentsBuilder
					.fromMethodName(FilesController.class, "serveFile", imagen).build().toUriString());
			
		}
		usuarioServicio.registrar(employee);
